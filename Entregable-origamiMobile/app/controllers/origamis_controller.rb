class OrigamisController < ApplicationController
  before_action :set_origami, only: [:show, :edit, :update, :destroy, :favourite_management]
  ...
  def favourite_management
    if(@origami.favourite)
      @origami.favourite = false
    else 
      @origami.favourite = true
    end
    @origami.save
    redirect_back fallback_location: origamis_path
  end
  ...
  private
    ...
    def origami_params
      params.require(:origami).permit(:name, :dificultad, :categoria, :imagen, :favourite)
    end
